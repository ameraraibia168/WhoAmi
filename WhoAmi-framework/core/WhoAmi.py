...
def sem():
 ...
 print"+ -- --=[ 10  exploits - 04  auxiliary - 00 network    ]"
...
def auxiliary():
  ...
  print "   04     radio_information                             connect with all radio"                                    
  print "" 
...
def radio_information():
 try:
  amerr = str(raw_input(default+'WhoAmi auxiliary('+red+'radio_information'+default+') > '))
  if  amerr[:10] == "set target" or  amerr[:10] == "set TARGET" :
     ip[0] = amerr[11:]
     print "TARGET => ", ip[0]
  elif amerr[:12] == "show options":
   print ""
   print ""
   print"    options (Radio information):" 
   print ""
   print"        Name      Current Setting      Required      Description"
   print "       no options enter run or exploit "
   print ""
   print ""
   radio_information()
   ...
   time.sleep(0.01)
   os.system("python3 lib/radio/radio.py")
   print basic_yellow + '[*] Done '
   print ''
...
Examples:
  ...
  use <name|index>
    ...
    if whoami[:35] == 'use payload_unix_python_reverse_tcp':
      ...
      gather_shodan_search() 
    elif whoami[:21] == "use radio_information":
      radio_information()
